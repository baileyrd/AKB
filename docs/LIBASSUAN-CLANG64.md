...
| Model fact | Value |
| --- | --- |
| Object | `library:gnupg:libassuan@clang64` |
| Kind | `library` |
| Status | `partial` |
| Confidence | `high` |
...
| Packaged as | `package:msys2:mingw-w64-clang-x86_64-libassuan` |
| Version (observed) | 2.5.7-1 |
| License (observed) | spdx:;LGPL-2.1-or-later;OR;GPL-3.0-or-later |
| Architecture (observed) | any |
| Installed size (observed) | 415.43 KiB |
...
```mermaid
flowchart LR
    subject["libassuan (CLANG64)"]
    d0["libgpg-error (CLANG64)"]
    subject -->|requires| d0
    style subject stroke-width:3px
```

Dependencies and dependents of `library:gnupg:libassuan@clang64` in the composed graph: 0 dependents and 1 dependency.
