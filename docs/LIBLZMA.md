...
| Installed size (observed) | 4594.05 KiB |
...
```mermaid
flowchart LR
    subject["liblzma (XZ Utils library)"]
    u0["GDB"]
    u0 -->|requires| subject
    u1["libarchive"]
    u1 -->|requires| subject
...
Dependencies and dependents of `library:tukaani:liblzma` in the composed graph: 2 dependents and 1 dependency.
