...
| 5 GNU Userland | [GNU userland role model](GNU-USERLAND-ROLE-MODEL.md), [GNU Bash](GNU-BASH.md), [GNU Coreutils](GNU-COREUTILS.md), [GNU Grep](GNU-GREP.md), [GNU Sed](GNU-SED.md), [GNU Awk (gawk)](GNU-AWK.md), [GNU Findutils](GNU-FINDUTILS.md) | Partial | Bash, Coreutils, Grep, Sed, Awk, and Findutils now have evidence-backed per-tool pages; archive/compression tools, editors/pagers/terminals, and SSH/curl/Git-adjacent tools still need per-tool startup, configuration, dependency, and interaction pages |
