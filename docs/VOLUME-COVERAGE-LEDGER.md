...
| 8 Toolchains | [Toolchain role model](TOOLCHAIN-ROLE-MODEL.md), [GCC](GNU-GCC.md), [GNU Binutils](GNU-BINUTILS.md), [GDB](GNU-GDB.md) | Partial | The GCC-oriented compiler/linker/debugger triad now has evidence-backed per-tool pages; Clang/LLD/LLDB, CMake/Meson/Ninja/pkgconf, and the Autotools family (autoconf/automake/libtool/make) still need per-tool modules, executable workflows, configuration, and observed build outputs |
