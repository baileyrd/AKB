...
| 5 GNU Userland | [GNU userland role model](GNU-USERLAND-ROLE-MODEL.md), [GNU Bash](GNU-BASH.md), [GNU Coreutils](GNU-COREUTILS.md) | Partial | Bash and Coreutils now have evidence-backed per-tool pages; grep/sed/awk/find, archive/compression tools, editors/pagers/terminals, and SSH/curl/Git-adjacent tools still need per-tool startup, configuration, dependency, and interaction pages |
